---
title: コアコンポーネントとヘッドレスを使用して魅力的なフォームを作成
description: コアコンポーネントとヘッドレスを使用して魅力的なフォームを作成します。
...
この実践ラボでは、AEM Sites と連携した最新のコアコンポーネントを備えた AEM Forms を使用して、アダプティブフォームを迅速に作成する方法を学習します。 これらのフォームをヘッドレスエクスペリエンスとして web、モバイル、チャットチャネルに配信し、シームレスなオムニチャネルデータキャプチャを実現します。 また、スタイル設定、カスタマイズ、フロントエンド開発に関するベストプラクティスについても学習します。
...
* AEM 6.5 Forms 環境で[ヘッドレスアダプティブフォームを有効にします](enable-headless-adaptive-forms-and-core-components.md)。
...
ビジネスユーザーは、アダプティブフォームエディターと標準のコアコンポーネントを使用して、アダプティブフォームを作成します。 その後、フォームを web、モバイル、チャットチャネルに配信して、データを取得できます。
...
1. アダプティブフォームの作成：
   ...
   1. コンテンツブラウザーを開いて、コンテナのガイドプロパティアイコンをクリックし、「**送信**」タブを開きます。

   1. **REST エンドポイントに送信**&#x200B;送信アクションを選択します

   1. 「**POSTリクエストを有効にする**」オプションを選択し、「**POST リクエストの URL**」テキストボックスにレッスン 2 で作成した REST エンドポイントを指定して、「**完了**」アイコンをクリックします。
...
1. アダプティブフォームを公開します。
   ...
   1. **アセットを公開**&#x200B;ダイアログで、「**公開**」をクリックします。 成功メッセージが表示されます。
...
1. **お問い合わせ** フォームを選択し、**編集** をクリックします。 アダプティブFormsエディターでフォームが開きます。


1. 「**連絡先番号**」フィールドを選択し、ツールバーの&#x200B;**編集アイコン（鉛筆アイコン）**&#x200B;をクリックします。 ポップアップツールバーが表示されない場合は、編集モードに切り替えます。 右上の「**編集**」ボタンをクリックし、左にある「**プレビュー**」ボタンをクリックします。
...
このレッスンでは、フロントエンド開発者として、様々なコンポーネントの代替 Material UI バージョンを作成する方法を学びます。 また、ビジネスユーザーが前に作成したアダプティブフォームにも適用します。
...
1. 自動的に更新されない場合は、ブラウザーを更新し、テキスト入力コンポーネントが別のバリアントを使用していることを確認します。
   ...
   この変更は、エンドユーザーに対して AEM Forms Server のフォーム定義に変更を加えずに行われ、検討中のヘッドレスチャネルに固有のものです。 例えば、このラボの web チャネルです。
...
* [アダプティブフォームコアコンポーネントの概要](https://experienceleague.adobe.com/ja/docs/experience-manager-core-components/using/adaptive-forms/introduction)
